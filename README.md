...
The expected first decision is `allow`; the second is `deny` with `consent_inactive`. `report.json` has zero false allows and zero false denies against the scenario's expected decisions. `receipts.jsonl` contains a SHA-256 chain bound to the scenario, policy and action digests. `assurance-result.json` is a compact, versioned cross-repository envelope that explicitly labels this run as a synthetic reference. Verification recomputes each policy decision and the envelope. The chain detects local editing if the scenario and expected root are retained or anchored externally; it is **not** a digital signature and does not establish who ran the test. Output directories are staged and existing directories are rejected rather than silently overwritten.
